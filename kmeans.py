...
# 3. Run data
def run_kmeans(data, cluster_count, random_seed=42):
    # Mengatasi nilai NaN dengan mengisi nilai rata-rata dari setiap kolom
    data = data.fillna(data.mean())

    kmeans = KMeans(n_clusters=cluster_count, random_state=random_seed)
    kmeans.fit(data)
...
if __name__ == "__main__":
    ...
    selected_columns = ["calories", "protein", "carbo"]  # Ganti dengan nama kolom yang sesuai
    ...
    # 4. Run Data dan Hasil
    dataset['Cluster'] = run_kmeans(selected_features, num_clusters, random_seed=42)
    print(dataset[['name', 'calories', 'protein', 'carbo', 'Cluster']])
